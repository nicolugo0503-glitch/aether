...
async function createNeonDB(apiKey, projectName) {
  ...
  const dbUrl = res.body?.connection_uris?.[0]?.connection_uri;
  if (!dbUrl) throw new Error('Could not get connection URI from Neon response');

  ok(`Database created → ${dbUrl.split('@')[1]?.split('/')[0] || 'neon.tech'}`);
  return dbUrl;
}
