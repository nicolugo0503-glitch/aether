// POST /api/agents/[id]/memory  — build / refresh agent memory from past runs
// GET  /api/agents/[id]/memory  — return current memory state
...
export async function POST(
  ...
  try {
    ...
    if (action === "toggle") {
      ...
      await prisma.agent.update({ where: { id }, data: { memoryEnabled: enabled ?? !agent.memoryEnabled } });
    ...
    if (runs.length < MIN_RUNS_FOR_MEMORY) {
      return NextResponse.json({
        ...
        runsRequired: MIN_RUNS_FOR_MEMORY,
      }, { status: 400 });
    }

    const memoryContext = await buildMemoryContext(
      agent.name, agent.role, agent.systemPrompt,
      runs.map(r => ({ input: r.input, output: r.output ?? "", status: r.status as "success" | "error", createdAt: r.createdAt })),
    );
